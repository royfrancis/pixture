...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      width = 9,
      pixture::pixcarouselOutput("carousel")
    )
  )
)
...
server <- function(input, output) {
    ...
    if (input$caption_check != "None") {
      ...
      div(
        ...
        selectInput(
          "valign",
          "Vertical alignment",
          choices = c("none", "top", "center", "bottom")
        ),
        selectInput(
          "halign",
          "Horiz alignment",
  ...
  output$carousel <- pixture::renderPixcarousel({
