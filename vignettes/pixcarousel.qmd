...
Adjust the number of images displayed per slide using `slides_to_show` and the number of images to scroll when navigating using `slides_to_scroll`. Set the border radius of the carousel using `border_radius` (in valid CSS units).
...
```{r}
pixcarousel(paths[1:4], scroll_lock = FALSE, rewind = TRUE)
```
